module ActivityTextValidation
  ...
  class_methods do
    def gibberish_text?(text, location: false, &token_skip)
      ...
      return true if tokens.any? { |token| !token_skipped?(token, token_skip) && gibberish_token?(token, location: location) }
      ...
      return true if vowel_count.to_f / letters.length < MIN_VOWEL_RATIO unless location
      return true if text.match?(CONSONANT_CLUSTER_PATTERN) && !location

      false
    end

    def gibberish_location?(text)
      gibberish_text?(text, location: true) { |token| location_abbreviation?(token) }
    end

    def location_abbreviation?(token)
      token_letters = token.gsub(/[^a-zA-Z]/, "")
      return false if token_letters.blank?

      LOCATION_ABBREVIATIONS.include?(token_letters.downcase)
    end

    def token_skipped?(token, token_skip)
      token_skip&.call(token)
    end

    def gibberish_token?(token, location: false)
      token_letters = token.gsub(/[^a-zA-Z]/, "")
      ...
      if token_letters.length >= 6 && token.match?(CONSONANT_CLUSTER_PATTERN)
        return false if location && token_vowels >= 2
