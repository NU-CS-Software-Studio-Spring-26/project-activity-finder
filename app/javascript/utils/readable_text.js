...
export function gibberishText(text) {
  ...
  if (tokensToCheck.some((token) => gibberishToken(token))) {
    return true
  }
  ...
  if (lettersVowelCount / letters.length < MIN_VOWEL_RATIO) return true
  if (CONSONANT_CLUSTER_PATTERN.test(text)) return true
